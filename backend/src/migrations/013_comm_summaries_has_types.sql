-- Add has_call and has_sms to get_comm_summaries so the Calls/SMS filters
-- show any patient who has ever had that comm type, not just whose last was.
-- DROP first because PostgreSQL won't let CREATE OR REPLACE change a function's return type.
DROP FUNCTION IF EXISTS public.get_comm_summaries(uuid);
